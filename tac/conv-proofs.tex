...
\end{align*}
Notice the following inequality that
\begin{align*}
...
=&\frac{1}{n^c} \frac{4}{\alpha} e^{-2\alpha \left(n^c\right)^2} \leq e^{-2\alpha \left(n^c\right)^2}
\end{align*}
thereby, as long as $n \geq \left(\frac{\ln2 - \ln\epsilon}{2\alpha}\right)^{\frac{1}{c}}$, we can satisfy 
$\int_{n^c}^\infty w\left(\mathbb{P} \left( X > s \right)\right) ds \leq \frac{\epsilon}{2}$, and obtain the following probability bound s.t. 
\begin{align*}
...
\leq &\,\,1 - \left(1 - e^{-2n^c}\right)^n \leq 1- \left(1- n e^{-2 n^c} \right)\\
=&\,\,n e^{-2 n^c},
>>>>>>> 24eabb3052b99fdd722a5f026787440fbc69761e
=======
Meanwhile, by invoking sub-Gaussian property, we infer that
\begin{align*}
&\,\,\mathbb{P}\left(u^+ \left(X_{[n]}\right) >  n^c \right) = 1- \mathbb{P}\left(u^+ \left(X_{[n]}\right) \le  n^c \right)\\
...
<<<<<<< HEAD
<<<<<<< HEAD
For subgaussian condition, notice that if we change the cutting bound $n^c$ by $\left(n\sqrt{\epsilon}\right)^c$ and repeat the same fashion of proof in proposition
\ref{prop:holder-dkw}, we can obtain a similar probability bound such that 
$$
...
$$
Applying the same procedure as above, the expected value of difference $\E \left|\overline \C_n- \C(X) \right|$ can be achieved by
$$\frac{2\Gamma\left(\frac{2}{c}\right)}{nc} + \frac{\Gamma\left(\frac{\alpha}{2\left(1-c\right)}\right)\left(2H\right)^{\frac{2}{\alpha}}}{2n^{\frac{\alpha-2c}{\alpha}}},$$
...
% \le \dfrac{1}{\epsilon^2} \E \left\|\sum_{n=k}^{\infty} \gamma_n \xi_n\right\|^2 
\le \frac{1}{\epsilon^2} \sum_{n=k}^{\infty} \gamma_n^2 \E\left\| \xi_n\right\|^2. \label{eq:b4}
\end{align}
The inequality above follows by observing that, for $m < n$, $\E(\xi_m \xi_n) = \E(\xi_m \E(\xi_n\mid \F_n))=0$.
