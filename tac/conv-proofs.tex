...
Under the assumption (A1), the term $u^+\left(X_{[n]}\right) w^+(\frac{1}{n})$ converges to 0,
henceforth, in conjunction with proposition \ref{prop:holder-quantile}, it suffices to show that
\begin{align*}
& \mathbb{P} \left( \left| \sum_{i=1}^{n-1} u^+\left(X_{[i]}\right) \left(w^+\left(\frac{n-i}{n} \right)  - w^+\left(\frac{n-i-1}{n} \right) \right) \right.\right. \\
