...
\begin{proof}(\textbf{\textit{Proposition \ref{prop:lower-bound}}})\ \\
	...
	Suppose that $w$ also satisfies the following condition: $|w(p) - w(\tilde{p})| \geq  |p - \tilde{p}|^\alpha$ for $p,\tilde p \in (0,1)$. Such a $w$ can be constructed as in the proof of Theorem 6 in \cite{aditya2016weighted}. \todoc{Can we just state what this function is?}
	...
	\end{align}
	where $P^n_{v}:= \otimes^n P_v$ is the joint distribution of $n$ samples from $P_v$ and \eqref{eq:pinsker} follows from Pinsker's inequality.
	We bound the KL-divergences as follows: \todoc{I changed the first $\le$ to $=$.}
	\begin{align*}
