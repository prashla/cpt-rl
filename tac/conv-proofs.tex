...
\subsection*{Proof of Proposition \ref{prop:cpt-finite}}
\begin{proof}
...
\begin{proof}
We shall focus on proving the first part of equation \eqref{eq:simple-estimation}. 
We have the following convergence claim holds almost everywhere:  % the following linear combination of simple functions: 
\begin{align}
...
\xi^+_{\frac{n-i-1}{n}}\right) \nonumber\\ 
& = \sum_{i=0}^{n-1} \xi^+_{\frac{i}{n}} \left(w^+\left(\frac{n-i}{n}\right)-
    w^+\left(\frac{n-i-1}{n}\right)\right).\label{eq:intsimp}
\end{align}
