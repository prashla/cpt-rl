...
%%%%
We state and prove a lemma below and this lemma will be used in the proof of Proposition \ref{prop:holder-asymptotic}.
\begin{proposition}
...
and hence the claim in \eqref{eq:simplew} follows.
...
The integral of LHS of \eqref{eq:simplew} can be simplified as follows:
\begin{align}
...
Without loss of generality, assume the  $w^+$ and $w^-$ are both 
$(1, \alpha)$ H\"{o}lder.
...
Now we find the upper bound for the probability of a single term in the sum above, i.e.,
\begin{align*}
...
For proving Proposition \ref{prop:holder-dkw}, we require DKW inequality, which we recall below.
...
\begin{proof}(\textbf{\textit{Proposition \ref{prop:lower-bound}}})\ \\
	...
	Let $X_v$, $v \in \{-1, +1\}$ denote a Bernoulli r.v. with underlying distribution $P_v$ defined by
	$$ P_{v} (X = 1) = 1 + v\delta^{\frac{1}{\alpha}} \quad v=+1,-1.$$
	Setting $w^+ = w^- = w$, where $w$ is \holder continuous with exponent $\alpha < 1$, 
	%(One could choose $w$ to be the weight function suggested by Tversky and Kahnemann, which is shown in Figure \ref{fig:w}). 
	we have 
	$$\C(P_{v}) = w(1 + v\delta^{\frac{1}{\alpha}}), \quad v=+1,-1.$$
	Suppose that $w$ also satisfies the following condition: $|w(p) - w(\tilde{p})| \geq  |p - \tilde{p}|^\alpha$ for $p,\tilde p \in (0,1)$. Such a $w$ can be constructed as in the proof of Theorem 6 in \cite{aditya2016weighted}. 
	Setting $p=1 + \delta^{\frac{1}{\alpha}}$ and $\tilde p = 1 - \delta^{\frac{1}{\alpha}}$, we obtain a separation of $\delta$ between $\C(P_{+1})$ and $\C(P_{-1})$, as shown below.
	\begin{align*}
		\left|\C(P_{+1}) - \C(P_{-1})\right| &= \left|w(p) - w(\tilde{p})\right| \ge \left|p - \tilde p\right|^\alpha  = \delta.
	\end{align*}	 
	Using the Le Cam method, we can lower bound the minimax error as follows:
	\begin{align}
		\cR_n(\cP) &  \ge \frac{\delta^2}{2}\left(1 - \tvnorm{P^n_{+1}- P^n_{-1}}\right) \nonumber\\
		&\ge \frac{\delta^2}{2}\left( 1 - \left(\frac12\dkl{P^n_{+}}{P^n_{-}}\right)^{\frac{1}{2}}\right), \label{eq:pinsker} 
	\end{align}
	where the final inequality follows from Pinsker's inequality.
	We bound the KL-divergences as follows:
	\begin{align*}
		& \dkl{P^n_{-}}{P^n_{+}} \le \frac{n}{2} \dkl{P_{+}}{P_{-}} \\
		& = \frac{n}{2} \left((1-\delta^{\frac1{\alpha}}) \log \frac{1 - \delta^{\frac1{\alpha}}}{1+ \delta^{\frac1{\alpha}}} + (1+\delta^{\frac1{\alpha}}) \log \frac{1-\delta^{\frac1{\alpha}}}{1+\delta^{\frac1{\alpha}}}\right) \\
		& = n \delta^{\frac1{\alpha}} \log \frac{1+\delta^{\frac1{\alpha}}}{1-\delta^{\frac1{\alpha}}} \le 3n \delta^{\frac2{\alpha}}
	\end{align*}
	where the first inequality uses chain rule of KL-divergences, the first equality follows by definition of KL-divergence between two Bernoulli distributions and the final inequality follows by using the fact that for small enough $\delta$, $\delta^{\frac1{\alpha}} \log \frac{1+\delta^{\frac1{\alpha}}}{1-\delta^{\frac1{\alpha}}} \le 3 \delta^{\frac{2}{\alpha}}$. 
	
	Plugging the bound on KL-divergences into \eqref{eq:pinsker}, we obtain
	\begin{align}
		\cR_n(\cP) &  \ge \frac{\delta^2}{2}\sqrt{3n} \delta^{\frac{1}{\alpha}}\ge \frac1{2^{2\alpha+3}n^\alpha },
	\end{align}
	for $\delta = \frac{1}{2^\alpha}\frac1{3n^{\frac{\alpha}{2}}}$.
	Hence proved.
\end{proof}
...
&= \E\left[\dfrac{\C(X^{\theta_n + \delta_n \Delta_n}) - \C(X^{\theta_n - \delta_n \Delta_n})}{2 \delta_n \Delta_n^{i}} \left.\right| \F_n\right] + \E\left[ \psi_n \mid \F_n\right],
\end{align}
where $\psi_n = \left(\dfrac{\epsilon^{\theta_n +\delta_n\Delta} - \epsilon^{\theta_n-\delta_n\Delta}}{2\delta_n\Delta_n^{i}}\right)$
...
$$\E \psi_n \rightarrow 0 \text{ a.s. as } n \rightarrow \infty.$$ 
Thus,
\begin{align}
&\E\left[\dfrac{\overline \C_n^{\theta_n+\delta_n \Delta_n} -\overline \C_n^{\theta_n-\delta_n \Delta_n}}{2 \delta_n \Delta_n^{i}} \left.\right| \F_n\right] \nonumber\\
...
$\C(X^\theta)$ is bounded a.s. for any parameter $\theta$ and the bias $\epsilon^\theta$ is bounded by Corollary \ref{cor:holder-dkw}.
