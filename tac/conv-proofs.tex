...
\begin{proof}(\textbf{\textit{Proposition \ref{prop:lower-bound}}})\ \\
	...
	Setting $u^+(x) = x, x\ge 0$, $w^+ = w^- = w$, where $w$ is \holder continuous with exponent $\alpha < 1$, 
	...
	\end{align*}	 
	Using Le Cam's method \cite{yu1997assouad}, we can lower bound the minimax error as follows:
	\begin{align}
		\cR_n(\cP) &  \ge \frac{\delta}{2}\left(1 - \tvnorm{P^n_{+1}- P^n_{-1}}\right) \nonumber\\
		&\ge \frac{\delta}{2}\left( 1 - \left(\frac12\dkl{P^n_{+1}}{P^n_{-1}}\right)^{\frac{1}{2}}\right), \label{eq:pinsker} 
	\end{align}
	where $P^n_{v}:= \otimes^n P_v$ is the joint distribution of $n$ samples from $P_v$. The inequality in \eqref{eq:pinsker} follows from Pinsker's inequality.
	We bound the KL-divergences as follows:
	\begin{align*}
		& \dkl{P^n_{-}}{P^n_{+}} \le n \dkl{P_{+}}{P_{-}} \\
	...
	\end{align*}
	where the first inequality uses chain rule of KL-divergences, the first equality follows by definition of KL-divergence between two Bernoulli distributions and the final inequality follows by using the fact that for $\delta^{\frac1{\alpha}} \le 1/2$, $\delta^{\frac1{\alpha}} \log \frac{1+\delta^{\frac1{\alpha}}}{1-\delta^{\frac1{\alpha}}} \le 3 \delta^{\frac{2}{\alpha}}$. 
	...
	\end{align}
	for $\delta =\frac1{6n^{\frac{\alpha}{2}}}$.
	Hence proved.
\end{proof}
