...
$$| w^+(x) - w^+(y) | \leq L_x |x-y|, \text{ for all } y \in (x-\rho_x,x+\rho_x). $$

% \subsubsection*{Main result}
\begin{proposition}
\label{prop:sample-complexity-discrete}
Assume (A1''). Let $L=\max\{L_k, k=1,\ldots,K\}$, where $L_k$ is the local Lipschitz constant of function $w^-(x)$ at points
$F_k$, where $k=1,\ldots,l$, and of function $w^+(x)$ at points $F_k$, $k=l+1,\ldots,K$. 
Let $M=\max\{u^{-}(x_k), k=1,\ldots,l\} \bigcup \{u^{+}(x_k), k=l+1,\ldots,K\}$ and $\rho =\min\{\rho_k\}$, where $\rho_k$ is as defined in (A1'').
