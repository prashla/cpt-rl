...
\subsection{Gradient algorithm using SPSA (CPT-SPSA-G)}
\label{sec:1spsa}
...
Let $\K' = \{\theta^* \mid \nabla_i \C(X^{\theta^*})=0, \forall i=1,\ldots,d\}$ denote the set of local minima of the CPT-value. If $\K'$ lies within the set $\Theta$ onto which the iterate $\theta_n$ (updated according to \eqref{eq:theta-update}) is projected, then the above theorem ensures that CPT-SPSA-G converges to $\K'$. However, it not possible to ensure that $\K' \subset \Theta$ in the stochastic optimization setting considered here, which implies that if $\K'$ lies outside $\Theta$, the iterate $\theta_n$ will get stuck on the boundary of $\Theta$.
