...
$\mathcal{H}\hspace*{-3pt}$, $\chi_{-1}=-\infty$.
...
for CPT-value optimization setting where we obtain only  asymptotically unbiased estimates
...
As in \cite{chang2013simulation}, we use only an elite portion of the candidate parameters that have been sampled, as it guides the parameter search procedure towards better regions more efficiently in comparison to an alternative that uses all the candidate parameters for updating $\eta$. 
