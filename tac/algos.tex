...
&+ \frac{(\epsilon_n^{\theta_n +\delta_n\Delta_n} - \epsilon_n^{\theta_n-\delta_n\Delta_n})}{2\delta_n\Delta_n^{i}}\bigg).
\end{align*}
Let $\zeta_n = \sum_{l = 0}^{n} \gamma_l \kappa_{l}$, where $\kappa_l = \frac{(\epsilon_l^{\theta_l +\delta_l\Delta_l} - \epsilon_l^{\theta_l-\delta_l\Delta_l})}{2\delta_l\Delta_l^{i}}$. Then, a critical requirement that allows us to ignore the bias term $\zeta_n$ is the following condition (see Lemma 1 in Chapter 2 of \cite{borkar2008stochastic}): 
...
Let $\K' = \{\theta^* \mid \nabla_i \C(X^{\theta^*})=0, \forall i=1,\ldots,d\}$ denote the set of local minima of the CPT-value. If $\K'$ lies within the set $\Theta$ onto which the iterate $\theta_n$ (updated according to \eqref{eq:theta-update}) is projected, then the above theorem ensures that CPT-SPSA converges to $\K'$. When it not possible to ensure that $\K' \subset \Theta$, the iterate $\theta_n$ will get stuck on the boundary of $\Theta$.
...
\label{sec:mras}
In this section, we provide a gradient-free algorithm (CPT-MPS) for maximizing the CPT-value, that is based on MRAS$_2$ from \cite{chang2013simulation}.
