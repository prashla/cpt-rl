...
with $\lambda = 0.25$ and $\sigma = 0.88$. Further, $w(p) = \frac{p^{0.61}}{(p^{0.61} + (1-p)^{0.61})^{1/0.61}}$. The choices for $\sigma$ and $w(\cdot)$ are based on the recommendations given by \cite{tversky1992advances}. 
...
\begin{align*}
\C\left( X \right) & = \intinfinity z^{\sigma} w^\prime \left(1 - F_{X^+}\left(z\right)\right) f_{X^+}\left(z\right)dz \\
&\quad - \lambda \intinfinity (-z)^{\sigma} w^\prime \left(1 - F_{X^-}\left(z\right)\right) f_{X^-}\left(z\right) dz,
\end{align*}
where, as mentioned before, $\sigma=0.88$ and $\lambda=0.25$.
