...
Let $\cP$ be a set of distributions. Let $\C(P)$ denote the CPT-value of a r.v. with distribution $P \in \cP$ and $\overline C(X_1,\ldots,X_n)$ denote an estimator of $\C(P)$, where $X_1,\ldots,X_n$ are samples from $P$. The minimax error $\cR_n(\cP)$ is defined by
\begin{align}
 \cR_n(\cP) := \inf_{\overline C} \sup_{P \in \cP} \E_P \l \overline C_n - \C(P) \r 
\end{align}


\begin{proposition}(\textbf{Lower bound})
	\label{prop:lower-bound}
For a set of distributions $\cP$ that includes the Bernoulli distribution,  the minimax error satisfies 
