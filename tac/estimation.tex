...
\end{align}
where $\overline \C_n$ is as defined in Algorithm \ref{alg:holder-est} and $\C(X)$ as in \eqref{eq:cpt-general}, and 
$\alpha$ is the \holder constant for $w^{\pm}$
\end{proposition}
...
For a set of distributions $\cP$ that has support within the interval $(0,1)$,  the minimax error satisfies 
