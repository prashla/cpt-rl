...
Furthermore, If the utilities functions are unbounded but satisfy sub-Gaussian property\footnote{A r.v. $X$ with mean $\mu$ is sub-Gaussian if $\exists \sigma >0$ such that $\E\left[e^{\lambda(X-\mu)}\right] \le e^{\sigma^2 \lambda^2/2}, \forall \lambda \in \R$.}, then $\forall \epsilon >0$ and $n \geq \left(\frac{\ln2-\ln\epsilon}{2\alpha}\right)^{\alpha+2}$, we have
\begin{align}
...
Further, if utilities satisfy sub-gaussian property, we obtain
