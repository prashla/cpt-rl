...
\end{align}
A similar claim holds with $u^-(X)$, $\xi^-_{\alpha}, w^-$ in place of  $u^+(X)$, $\xi^+_{\alpha}, w^+$, respectively. 
...
Consider the special case when $w^+(p)=w^-(p)=p$ and $u^+$ ($-u^-$), when restricted to the positive (respectively, negative) half line, are the identity functions. In this case, the CPT-value predictor $\overline \C_n$ coincides with the sample mean estimator for regular expectation. 
...
In this section, we establish that the CPT-value predictor $\overline \C_n$ is asymptotically consistent when the weights are Lipschitz continuous,  i.e., the following assumption in place of (A1):
