...
$X_{[1]}, X_{[2]}, \ldots ,X_{[n]}$. Since we assume $u^+$ is increasing (see (A2) below),  $u^+(X_{[1]}),\ldots ,u^+(X_{[n]})$ are in ascending order.
...
\vspace{-0.5ex}
\State Apply $u^{-}$ on the sequence $\{X_{[1]}, X_{[2]}, \ldots ,X_{[n]}\}$; since we assume $u^{-}$ is decreasing (see (A2) below), $u^{-}(X_{[1]}), \ldots, u^{-}(X_{[n]})$ are in descending order.     
\State Let
\vspace{-0.5ex}
...
Let $X_{[1]}, X_{[2]}, \ldots ,X_{[n]}$ denote the order statistics sampled from the distribution of $X$. 
...
&= \sum_{i=1}^{n-1} w^+\left(\frac{n-i}{n}\right) \left(u^+\left(X_{[i+1]}\right) - u^+\left(X_{[i]} \right)\right)\\
