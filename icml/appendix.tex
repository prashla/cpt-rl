...
$w^{+}(1-\hat{F}_n(x)) \leq L (1-\hat{F}_n(x))$,  
for all $n$ and 
...
\begin{equation}
\intinfinity (1-{\hat F_n}^+(x))  dx =\intinfinity \int_x^\infty d \hat{F}_n(t) dx.\label{eq:2}
\end{equation}
...
\begin{equation}
\intinfinity \int_x^\infty d \hat{F}_n(t) dx = \intinfinity \int_0^t dx d \hat{F}_n(t) = \intinfinity t d\hat{F}_n(t) = \frac{1}{n} \sum_{i=1}^n u^+(X_{[i]}),
 \end{equation}
...
From the foregoing,
$$
\lim_{n\rightarrow \infty} \intinfinity L\cdot(1-\hat{F}_n(x)) dx
\overset{a.s} \longrightarrow
\intinfinity L \cdot(P(u^+(X)>x)) dx.$$
Hence, we have 
$$
\int_0^\infty w^{(+)}(1-\hat{F}_n(x)) dx \xrightarrow{a.s.} 
