...
travelers' satisfaction, a problem many may agree is 
 still inadequately addressed today, at least in big cities.
...
So we are inconsistent with these people.}
	...
	and, as it appears, the psychology literature agrees on that human preferences 
	are inconsistent with expected utilities no matter what nonlinearities are used
...
More precisely, we will use the so-called \emph{cumulative prospect theory} (CPT),
 	a later, refined variant of prospect theory due to \cite{tversky1992advances}, 
	which is even more supported than prospect theory \citep[e.g.,][]{Barberis:2012vs}.
...
The first challenge stems from the fact that the CPT-value assigned to a random variable is defined through a nonlinear transformation of certain cumulative distribution functions associated with the random variable (cf. \cref{sec:cptdef} for the definition). 
...
Under certain technical assumptions, we prove consistency and sample complexity bounds, the latter based on the
 DvoretzkyÐKieferÐWolfowitz theorem.
