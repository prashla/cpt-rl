...
\todoc{Does this assume that $u^+(x)=0$ for $x<0$?}
...
\todoc{Asymptotic convergence is called asymptotic consistency, because convergence itself does not imply that the limiting
value will be the correct one.}
...
\end{proposition}
\begin{proof}
See Appendix \ref{sec:holder-proofs}. \todoc{It seems that the statement of the proposition is not consistent with the proof.
The proof assumes H\"older continuity.}
\end{proof}
\todoc{We had here: ``While the above result establishes that $\overline \C_n$ is an unbiased estimator in the asymptotic sense, ''
This is not true. For this, we would need to prove that $\EE{ \overline \C_n } \to \C(X)$, which requires separate proof.}
