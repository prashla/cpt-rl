...
indexed by
$u=(u^+,u^-)$, $w=(w^+,w^-)$, where $u^+,u^-:\R\rightarrow \R_+$ and $w^+,w^-:[0,1] \rightarrow [0,1]$ are continuous (see assumptions (A1)-(A2) in Section \ref{sec:cpt-sampling} for precise requirements on $u$ and $w$),
 is defined as \todoc{I would prefer $\Prob{\cdot}$ to $P(\cdot)$ everywhere.}
\begin{align}
\C_{u,w}(X) &= \intinfinity w^+(P(u^+(X)>z)) dz\nonumber \\
&\quad - \intinfinity w^-(P(u^-(X)>z)) dz\,. \label{eq:cpt-general}
\end{align}
For notational convenience, when $u,w$ is fixed, we drop the dependence on them and use $\C(X)$ to denote the CPT-value of $X$. \todoc{This is a bit wild here. I think we later need $u^+(x)=0$ for $x<0$ and $u^-(x)=0$ for $x>0$. 
Then we should say this here. The figure needs to be modified. Further, the integral should better go from $-\infty$ to $+\infty$.
In both cases.}
Note that when $w^+,w^-$
...
$\C(X) = \intinfinity P(X>z) dz -  \intinfinity P(-X>z)) dz = \EE{ (X)^+ } - \EE{ (X)^- }$, showing the connection to expectations.
...
consider a scenario where one can either earn \$$500$ w.p $1$ or earn \$$1000$ with probability (w.p.) $0.5$ and nothing otherwise. 
