...
%\begin{tikzpicture}
  ...
  %}
  %]
  %\pic (feeling small) [fill=red,blue] {person};
%\end{tikzpicture}
...
\draw [->,thick] (tmp) --   (tmp3);
\draw [->,thick,dotted] (tmp4)  |- node[below left=0.1cm and 1cm] {\textbf{CPT preferences}} (belowagent) -|  (agent.south);
\end{tikzpicture}}
...
\For{$n = 0,1,2,\ldots$}	
	...
	\PEval
	    ...
	    \State Obtain CPT-value estimate $\widehat V_n^{(\pi_n+\delta_n (\Delta_n+\hat \Delta_n))}(x^0)$ using Algorithm \ref{alg:holder-est}
	    \EndPEval
	    \PEvalPrime
  	    \State Simulate $m_n$ episodes of the SSP using policy $(\pi_n-\delta_n (\Delta_n + \hat \Delta_n))$
	    \State Obtain CPT-value estimate $\widehat V_n^{\pi_n-\delta_n (\Delta_n+\hat\Delta_n)}(x^0))$ Algorithm \ref{alg:holder-est}
	    \EndPEvalPrime
	    	    \PEvalPrimeDouble
  	    \State Simulate $m_n$ episodes of the SSP using policy $\pi_n$
	    \State Obtain CPT-value estimate $\widehat V_n^{\pi_n}(x^0))$ using Algorithm \ref{alg:holder-est}
	    \EndPEvalPrimeDouble
	    \PImpNewton
...
%Let $\varphi>0$ be a positive constant satisfying the condition that the set $\big\{\pi:\mathcal{H}(\C(D^\pi)\geq \frac{1}{\varphi} \big\}$ has a strictly positive Lebesgue/counting measure\index{Lebesgue measure}\index{counting measure}.
