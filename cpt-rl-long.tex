...
=======
\State Simulate $n$ samples $X_1,\ldots, X_n$ of the r.v. $X$. 
\State Let $X_{[1]}, \ldots, X_{[n]}$ denote the order statistics. Let $U^+_{[i]} = u^+(X_{[i]})$ and $U^-_{[i]} = u^-(X_{[i]})$, $i=1,\ldots,n$.
...
\State Return 
$$\hat V_n(X):=\sum_{i=1}^{n-1} U^+_{[i]} \left(w^+\left(\frac{n-i+1}{n}\right)- w^+\left(\frac{n-i}{n}\right)\right) - \sum_{i=1}^{n-1} U^-_{[i]} \left(w^-\left(\frac{n-i+1}{n}\right)- w^-\left(\frac{n-i}{n}\right)\right).$$
>>>>>>> origin/master
...
\widehat V_n(X) \rightarrow V(X) \text{ a.s. as } n \rightarrow \infty,
\end{align}
where $\widehat V_n(X)$ is as defined in Algorithm \ref{alg:holder-est}.
>>>>>>> origin/master
\end{proposition}
\begin{proof}
...
=======
P(\left |\widehat V_n(X)-  V(X) \right| \leq  \epsilon ) > \delta\text{     ,} \forall n \geq \ln(\frac{1}{\delta})\cdot 
...
Assume the function $w^+(x)$ and $w^-(x)$ are Lipchitz continuous, then the asymptotic convergence property in proposition
\ref{prop:holder-asymptotic} holds, and in addition, we have 
$$
P(\left |\hat V_n(X)- V(X) \right| \leq  \epsilon ) > \delta\text{     ,} \forall n \geq \ln(\frac{1}{\delta})\cdot 
