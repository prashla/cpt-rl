...
\begin{align}
\left| \E\left[\left.\dfrac{\widehat V_n^{\theta_n + \delta_n \Delta_n}(x^0) -\widehat V_n^{\theta_n - \delta_n \Delta_n}(x^0)}{2 \delta_n \Delta_n^{i}}\right| \F_n \right] - \nabla_{i} V^{\theta_n}(x^0) \right| \rightarrow 0 \text{ as } n\rightarrow\infty.
\end{align} 
\end{lemma}
\begin{proof}
...
We claim
\begin{align}
\quad\E\left[\dfrac{\widehat V_n^{\theta_n + \delta_n \Delta_n}(x^0) -\widehat V_n^{\theta_n - \delta_n \Delta_n}(x^0)}{2 \delta_n \Delta_n^{i}} \left.\right| \F_n\right] 
= \quad&\E\left[\dfrac{V_n^{\theta_n + \delta_n \Delta_n}(x^0) -V_n^{\theta_n - \delta_n \Delta_n}(x^0)}{2 \delta_n \Delta_n^{i}} \left.\right| \F_n\right] + \E\left[ \eta_n \mid \F_n\right],
\end{align}
...
From Proposition \ref{prop:holder-dkw}, we see that, $\epsilon^\theta = LM\sqrt{\frac{2 \log \log m_n}{m_n}}$, provided $m_n$ is sufficiently large.
...
\begin{align}
\quad\E\left[\dfrac{\widehat V_n^{\theta_n + \delta_n \Delta_n}(x^0) -\widehat V_n^{\theta_n - \delta_n \Delta_n}(x^0)}{2 \delta_n \Delta_n^{i}} \left.\right| \F_n\right] 
&\xrightarrow{n\rightarrow\infty}  \E\left[\dfrac{V^{\theta_n + \delta_n \Delta_n}(x^0) -V^{\theta_n - \delta_n \Delta_n}(x^0)}{2 \delta_n \Delta_n^{i}} \left.\right| \F_n\right].  \label{eq:l1}
\end{align}
We now analyse the RHS of the above.
By using suitable Taylor's expansions,
\begin{align*}
V^{\theta_n + \delta_n \Delta_n}(x^0) = V^{\theta_n}(x^0) + \delta_n \Delta_n\tr \nabla V^{\theta_n}(x^0) + \frac{\delta^2}{2} \Delta_n\tr \nabla^2 V^{\theta_n}(x^0)\Delta_n + O(\delta_n^3), \\
V^{\theta_n - \delta_n \Delta_n}(x^0) = V^{\theta_n}(x^0) - \delta_n \Delta_n\tr \nabla V^{\theta_n}(x^0) + \frac{\delta^2}{2} \Delta_n\tr \nabla^2 V^{\theta_n}(x^0)\Delta_n + O(\delta_n^3).\end{align*}
...
\begin{align}
\theta^{i}_{n+1}  =  \theta^{i}_n -  \gamma_n(\nabla_{i} V^{\theta_n}(x_0) + \beta_n + \xi_n), 
\label{eq:1spsa-equiv}
\end{align}
where 
\begin{align*}
\beta_n \quad= &\quad \E\left(\dfrac{(\widehat V^{\theta_n + \delta_n \Delta_n}(x^0) -\widehat V^{\theta_n - \delta_n \Delta_n}(x^0))}{2 \delta_n \Delta_n^{i}} \mid \F_n \right) - \nabla \C(D^\theta), \text{ and}\\
\xi_n \quad = & \quad\left(\dfrac{\widehat V^{\theta_n + \delta_n \Delta_n}(x^0) -\widehat V^{\theta_n - \delta_n \Delta_n}(x^0)}{2 \delta_n\Delta_n^{i}}\right)  - \E\left(\dfrac{(\widehat V^{\theta_n + \delta_n \Delta_n}(x^0) -\widehat V^{\theta_n - \delta_n \Delta_n}(x^0))}{2 \delta_n \Delta_n^{i}}\mid \F_n \right).
\end{align*}
...
\item $\nabla \C(D^\theta)$ is a continuous $\R^{\L\K}$-valued function.
...
\begin{align}
\dot\theta^{i}_t = \check\Gamma_{i}\left( \nabla V^{\theta^{i}_t}(x^0)\right), \text{ for} i=1,\dots,\L\M,\label{eq:pi-ode}
\end{align}
\end{enumerate} 
...
\begin{align}
\E\left\| \xi_n\right\|^2\le &\E\left(\dfrac{\widehat V^{\theta_n + \delta_n \Delta_n}(x^0) -\widehat V^{\theta_n - \delta_n \Delta_n}(x^0)}{2 \delta_n \Delta_n^i}\right)^2 \label{eq:mi}\\
\le &\left(\left(\E\left(\dfrac{\widehat V^{\theta_n + \delta_n \Delta_n}(x^0)}{2 \delta_n \Delta_n^i}\right)^2\right)^{1/2}
+ \left(\E\left(\dfrac{\widehat V^{\theta_n - \delta_n \Delta_n}(x^0)}{2 \delta_n \Delta_n^i}\right)^2\right)^{1/2}\right)^2 \label{eq:minko}\\
\le &\frac1{4\delta_n^2} \left[ \E\left(\frac1{(\Delta_n^i)^{2+2\alpha_1}}\right) \right]^{\frac{1}{1+\alpha1}} \nonumber\\
& \times \left(\left[\E\left[ (\widehat V^{\theta_n + \delta_n \Delta_n}(x^0))\right]^{2+2\alpha_2}\right]^{\frac{1}{1+\alpha_2}} +
\left[\E\left[ (\widehat V^{\theta_n - \delta_n \Delta_n}(x^0))\right]^{2+2\alpha_2}\right]^{\frac{1}{1+\alpha_2}}\right)\label{eq:holder}\\
\le &\frac1{4\delta_n^2} \left(\left[\E\left[ (\widehat V^{\theta_n + \delta_n \Delta_n}(x^0))\right]^{2+2\alpha_2}\right]^{\frac{1}{1+\alpha_2}} +
\left[\E\left[ (\widehat V^{\theta_n - \delta_n \Delta_n}(x^0))\right]^{2+2\alpha_2}\right]^{\frac{1}{1+\alpha_2}}\right) \label{eq:h2}\\
...
\item The set $\K = \{\theta \mid \check\Gamma_{i} \left(\nabla V^{\theta}(x^0)\right)=0, \forall i=1,\ldots,\L\M\}$ serves as the asymptotically stable attractor for the ODE \eqref{eq:pi-ode}.
\end{itemize} 
...
\begin{align}
    \left| \E\left[\left.\dfrac{\widehat V_n^{\theta_n+\delta_n(\Delta_n+\widehat\Delta_n)}(x^0) + \widehat V_n^{\theta_n-\delta_n(\Delta_n+\widehat\Delta_n)}(x^0) - 2 \widehat V_n^{\theta_n}(x^0)}{\delta_n^2 \Delta_n^{i}\widehat\Delta_n^j}\right| \F_n \right] - \nabla^2_{i,j} V^{\theta_n}(x^0) \right| \rightarrow 0 \text{ as } n\rightarrow\infty.
\end{align} 
\end{lemma}
\begin{proof}
...
Before we prove Theorem \ref{thm:2spsa}, we show that the Hessian recursion \eqref{eq:2spsa-H} in the main paper converges to the true Hessian, for any policy $\theta$.

\begin{lemma}
\label{lemma:h-est}
For any $i, j= 1,\ldots,\L\M$, we have almost surely,  
$$\left \| H^{i, j}_n - \nabla^2_{i,j} V^{\theta_n}(x^0) \right \| \rightarrow 0,
\text{ and }\left \| \Upsilon(\overline H_n)^{-1} - \Upsilon(\nabla^2_{i,j} V^{\theta_n}(x^0))^{-1} \right \| \rightarrow 0.
$$
\end{lemma}
\begin{proof}
...
For $i=1,\ldots, \L\M$
\begin{align}
 \theta^{i}_{n+1} =& \Gamma_{i}\left(\theta^{i}_n - \gamma_n \sum_{j=1}^{\L\M} \bar M^{i,j}(\theta_n) \nabla_{j} V^\theta_n(x^0) + \gamma_n \zeta_n + \chi_{n+1} - \chi_n \right), \label{eq:pi-n}
\end{align}
where 
\begin{align*}
\bar M^{i,j}(\theta) = &\Upsilon(\nabla^2 V^{\theta}(x^0))^{-1}\\
 \chi_n =& \sum_{m=0}^{n-1} \gamma_m \sum_{k=1}^{\L\M} {\bar{M}}_{i,k}(\theta_m)\Bigg(
\frac{V^{\theta_m-\delta_m\Delta_m - \delta_m\widehat\Delta_m}(x^0) -
V^{\theta_m+\delta_m\Delta_m + \delta_m\widehat\Delta_m}(x^0)}{2\delta_m \Delta^k_m} 
 \\
 &- E\left[\frac{V^{\theta_m-\delta_m\Delta_m - \delta_m\widehat\Delta_m}(x^0) -
V^{\theta_m+\delta_m\Delta_m + \delta_m\widehat\Delta_m}(x^0)}{2\delta_m \Delta^k_m} 
...
$$
\dot\theta^{i}_t = \check\Gamma_{i}\left( \nabla V^{\theta^{i}_t}(x^0) \Upsilon(\nabla^2 V^{\theta_t}(x^0))^{-1} \nabla V^{\theta^{i}_t}(x^0) \right).$$
Further, $\K = \{\theta \mid
\nabla V^{\theta^{i}}(x^0)  \check\Gamma_{i}\left(\Upsilon(\nabla^2 V^{\theta}(x^0))^{-1} \nabla V^{\theta^{i}}(x^0)\right)
...
\end{lemma}
\begin{proof}
    Let $\mathcal{A}_n$ be the event that either the first if statement (see \ref{step:3a}) is true or the second if statement in the else clause (see \ref{step:3b}) is true within the Thresholding step of Algorithm \ref{alg:mras}. Let $\mathcal{B}_n:=\{V^{\theta^*_{n}}(x^0)-V^{\theta^*_{n-1}}(x^0)\leq \frac{\varepsilon}{2}\}$.
    Whenever $\mathcal{A}_n$ holds, we have $\widehat V_n^{\theta^*_{n}}(x^0)-\widehat V_n^{\theta^*_{n-1}}(x^0)\geq \varepsilon$ and hence, we obtain

\begin{align*}
P(\mathcal{A}_{n}\cap \mathcal{B}_n) \leq &
P 
\left(\big\{\widehat V_n^{\theta^*_{n}}(x^0)-\widehat V_{n-1}^{\theta^*_{n-1}}(x^0)\geq \varepsilon\big\} \cap \left\{V^{\theta^*_{n}}(x^0)-V^{\theta^*_{n-1}}(x^0)\leq \frac{\varepsilon}{2}\right\}\right)\\
 \leq &
P\Big(\bigcup_{\theta\in\Lambda_n, \theta'\in \Lambda_{n-1}} \Big\{\big\{\widehat V_n^{\theta}(x^0)-\widehat V_{n-1}^{\theta'}(x^0)\geq \varepsilon\big\} \cap \left\{\C(D^\theta)-V^{\theta'}(x^0)\leq \frac{\varepsilon}{2}\right\} \Big\}\Big)\\
\leq & \sum_{{\theta\in \Lambda_n},{\theta'\in \Lambda_{k-1}}}P\left(\big\{\widehat V_n^{\theta}(x^0)-\widehat V_{n-1}^{\theta'}(x^0)\geq \varepsilon\big\} \cap
\left\{\C(D^\theta)-V^{\theta'}(x^0)\leq \frac{\varepsilon}{2}\right\}\right) \\
\leq & |\Lambda_n||\Lambda_{n-1}|\sup_{\theta,\theta'\in \Theta}P\left(\big\{\widehat V_n^{\theta}(x^0)-\widehat V_{n-1}^{\theta'}(x^0)\geq \varepsilon\big\} \cap
\left\{\C(D^\theta)-V^{\theta'}(x^0)\leq \frac{\varepsilon}{2}\right\} \right) \\
\leq &|\Lambda_n||\Lambda_{n-1}|\sup_{\theta,\theta'\in \Theta}P\left(\widehat V_n^{\theta}(x^0)-\widehat V_{n-1}^{\theta'}(x^0)-\C(D^\theta)+V^{\theta'}(x^0)\geq \frac{\varepsilon}{2}\right) \\
\leq &|\Lambda_n||\Lambda_{n-1}|\sup_{\theta,\theta'\in \Theta}\left(P\left(\widehat V_n^{\theta}(x^0)-\C(D^\theta)\geq \frac{\varepsilon}{4}\right) + P\left(\widehat V_{n-1}^{\theta'}(x^0)-V^{\theta'}(x^0)\geq \frac{\varepsilon}{4}\right)\right) \\
...
\begin{align*}
\sum_{n=1}^{\infty}\big[V^{\theta^*_n}(x^0)-V^{\theta^*_{n-1}}(x^0)\big] 
=&\sum_{n:~\mathcal{A}_n \text{occurs}}\big[V^{\theta^*_n}(x^0)-V^{\theta^*_{n-1}}(x^0)\big]+\sum_{n:~\mathcal{A}_n^c \text{occurs}}\big[V^{\theta^*_n}(x^0)-V^{\theta^*_{n-1}}(x^0)\big] \\ 
=&\sum_{n:~\mathcal{A}_n \text{occurs}}\big[V^{\theta^*_n}(x^0)-V^{\theta^*_{n-1}}(x^0)\big]\\ 
=&\sum_{n:~\mathcal{A}_n\cap \mathcal{B}_n \text{occurs}}\big[V^{\theta^*_n}(x^0)-V^{\theta^*_{n-1}}(x^0)\big]\\
&\quad +\sum_{n:~\mathcal{A}_n\cap \mathcal{B}_n^c \text{occurs}}\big[V^{\theta^*_n}(x^0)-V^{\theta^*_{n-1}}(x^0)\big] \\ 
...
%
From the last equality above, we conclude that it is a contradiction because, $\C(D^\theta) > V^{\theta^*}(x^0)$ for any $\theta$ (since $\theta^*$ is the global minimum). The main claim now follows since $\mathcal{A}_n$ can happen only a finite number of times.
\end{proof}
